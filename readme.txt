...
19_PDI
	...
	The reset capactor in the target device has to be disconnected. Otherwise the PDI programmer cannot work normally because the signal in data line is distorted.
	It is found that UART can USART cannot be in the same port, otherwise they don't work normally. UART1 is ansynchronize and UART0 is synchronize, the output of UART1 is abnormal when UART0 works. The reason is unknown.
	The USART for PDI can be changed in ECD300_pdi_physic.c
	The UART3 is usded for PDI programming at present. Plug the 3-line connector to  UART3, with the orange line connecting to the power line. And plug the white line to DE1 and black line to GND.
...
21_JbiPlayer
	...
	PIN3 of PORTD controls the behaviour of this application. If this pin is connected to GND, then the application receives the JBC file from the host through a virtual port emulated by this device, and save the data received to NAND flash. If this pin is left unconnected, the application reads the JBC content from NAND flash and programs it to the target device through JTAG interface. 
	DB7 to DB4 are the four pins of JTAG at present.
	22_JbcFileSender cooperates with this application to transfer the JBC file from host to ECD300.

22_JbcFileSender
	JbcFileSender runs in the computer and sends the content of a JBC file to the designated serial port. In Win7, the serial port can only be opened when it is enummed as COM9, the reason is unknown yet.