...
*/
static JBIE_JTAG_STATE jbi_jtag_state = JBI_ILLEGAL_JTAG_STATE;

/*
*	Store current stop-state for DR and IR scan commands
*/
static JBIE_JTAG_STATE jbi_drstop_state = IDLE;
static JBIE_JTAG_STATE jbi_irstop_state = IDLE;

/*
*	Store current padding values
*/
static unsigned int jbi_dr_preamble  = 0;
static unsigned int jbi_dr_postamble = 0;
static unsigned int jbi_ir_preamble  = 0;
static unsigned int jbi_ir_postamble = 0;
static unsigned int jbi_dr_length    = 0;
static unsigned int jbi_ir_length    = 0;
static unsigned char *jbi_dr_preamble_data  = NULL;
static unsigned char *jbi_dr_postamble_data = NULL;
static unsigned char *jbi_ir_preamble_data  = NULL;
static unsigned char *jbi_ir_postamble_data = NULL;
static unsigned char *jbi_dr_buffer         = NULL;
static unsigned char *jbi_ir_buffer         = NULL;
...
*/
static struct JBIS_JTAG_MACHINE
{
...
*/
static unsigned short jbi_jtag_path_map[16] =
{
...
(
	...
	unsigned long preamble_data_address
)
...
/****************************************************************************/
{
	...
	if (status == JBIC_SUCCESS)
	{
		for (i = 0; i < count; ++i)
		{
			...
			if (preamble_data_address == NULL)
			{
				jbi_dr_preamble_data[i >> 3] |= (1 << (i & 7));
			}
			else
			{
				if (jbi_read_char_array(preamble_data_address, j >> 3) & (1 << (j & 7)))
				{
					jbi_dr_preamble_data[i >> 3] |= (1 << (i & 7));
				}
				else
...
(
	unsigned int count,
	unsigned int start_index,
	unsigned long postamble_data_address
)
...
/****************************************************************************/
{
	...
	if (status == JBIC_SUCCESS)
	{
		for (i = 0; i < count; ++i)
		{
			...
			if (postamble_data_address == NULL)
			{
				jbi_dr_postamble_data[i >> 3] |= (1 << (i & 7));
			}
			else
			{
				if (jbi_read_char_array(postamble_data_address, j >> 3) & (1 << (j & 7)))
				{
					jbi_dr_postamble_data[i >> 3] |= (1 << (i & 7));
				}
				else
...
/****************************************************************************/
{
	...
	for (; i < k; ++i, ++j)
	{
		if (jbi_read_char_array(target_data_address, j >> 3L) & (1L << (j & 7L)))
		{
			buffer[i >> 3L] |= (1L << (i & 7L));
		}
		else
		{
			buffer[i >> 3L] &= ~(unsigned char) (1L << (i & 7L));
		}
	}
...
/****************************************************************************/
{
	...
	for (i = start_index; i < k; ++i, ++j)
	{
		if (buffer[j >> 3] & (1 << (j & 7)))
		{
			jbi_write_char_array(target_data_address, i>>3, jbi_read_char_array(target_data_address, i>>3)|(1 << (i & 7)));
		}
		else
		{
			jbi_write_char_array(target_data_address, i>>3, jbi_read_char_array(target_data_address, i>>3)&(~(unsigned char) (1 << (i & 7))));
		}
	}
}
...
(
	unsigned int count,
	unsigned long tdi_data_address,
	unsigned int start_index
)
...
(
	unsigned int count,
	unsigned long in_data_address,
	unsigned int in_index,
	unsigned long out_data_address,
