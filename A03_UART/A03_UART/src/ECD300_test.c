...
void _showSpiRegisters(SPI_t *spi)
{
    printString("ctrl:  ");printHex(spi->CTRL);printString("\r\n");
    printString("intctrl:  ");printHex(spi->INTCTRL);printString("\r\n");
    printString("status:  ");printHex(spi->STATUS);printString("\r\n");
    printString("data:  ");printHex(spi->DATA);printString("\r\n");
}

void _showPowerReduction()
{
	printString("PR_PRGEN: "); printHex(PR_PRGEN); printString("\r\n");
	printString("PR_PRPA: "); printHex(PR_PRPA); printString("\r\n");
	printString("PR_PRPB: "); printHex(PR_PRPB); printString("\r\n");
	printString("PR_PRPC: "); printHex(PR_PRPC); printString("\r\n");
	printString("PR_PRPD: "); printHex(PR_PRPD); printString("\r\n");
	printString("PR_PRPE: "); printHex(PR_PRPE); printString("\r\n");
	printString("PR_PRPF: "); printHex(PR_PRPF); printString("\r\n");
}

void ecd300TestSpi(void)
{
    usart_rs232_options_t uartOption;
	unsigned long i;
	char rc;
	...
	//SPI master
    printString("Start initializing SPI in PORTE as master\r\n");
    rc = spi_e_init(true, 1000000, 1, SPI_MODE_0);
	printHex(rc);
	printString("\r\n");
    printString("Finish initializing SPI in PORTE\r\n");
    _showSpiRegisters(&SPIE);
	   
	_showPowerReduction();
	   
	//SPI slave
	printString("\r\nStart initializing SPI in PORTF as slave\r\n");
	rc = spi_f_init(false, 1000000, 3, SPI_MODE_0);
	printHex(rc);
	printString("\r\n");
	printString("Finish initializing SPI in PORTF\r\n");
	_showSpiRegisters(&SPIF);

	//SPI bytes.
	{
		printString("\r\n**** send 16 byte from SPIE to SPIF ****\r\n");
		printString("Connect DE4 to DF4 as SS\r\n");
		printString("Connect DE5 to DF5 as MOSI, DE6 to DF6 as MISO, DE7 to DF7 as SCK\r\n");
		printString("press enter key to continue\r\n");
		getChar();
		
		unsigned char masterBuffer[15] = {0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff};
		unsigned char slaveBuffer[15] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f};
		
		printString("master data sent:\r\n");
		for(int i=0; i<15; i++)
		{
			printHex(masterBuffer[i]);
			printString(", ");
		}
		printString("\r\n");
		printString("slave data sent:\r\n");
		for(int i=0; i<15; i++)
		{
			printHex(slaveBuffer[i]);
			printString(", ");
		}
		printString("\r\n");
		rc = spi_f_send_data(slaveBuffer, 15);
		printString("slave return value: "); printHex(rc); printString("\r\n");
		rc = spi_e_send_data(masterBuffer, 15);
		printString("master return value: "); printHex(rc); printString("\r\n");
		for(;;)
		{
			if(spi_e_send_finished())
			break;
		}
		printString("master data received:\r\n");
		for(int i=0; i<15; i++)
		{
			printHex(masterBuffer[i]);
			printString(", ");
		...
		printString("\r\n");
		rc = spi_f_slave_receive_data(slaveBuffer, 15);
		printString("slave received "); printHex(rc); printString(" bytes\r\n");
		if(rc > 0)
		{
			printString("slave data received:\r\n");
			for(int i=0; i<rc; i++)
			{
				printHex(slaveBuffer[i]);
				printString(", ");
