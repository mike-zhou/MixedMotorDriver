...
#define EXTERNAL_RAM_BASE	0x7f000000
#define JAM_FILE_BASE		0x7f800000
...
int CECD100AppDlg::executeJam(unsigned char *action)
{
	...
	{
		{
			if(file.GetLength()!=file.Read(jamBuffer, file.GetLength()))
			{
				MessageBox(ERROR_WHEN_READING_JAM_FILE, APPLICATION_TITLE, MB_OK|MB_ICONERROR);
			}
			else
			{
				jp_main(JAM_FILE_BASE, file.GetLength(), action, 1);
			}
		}
		file.Close();
	}
...
int CECD100AppDlg::jtagInit(void)
{
	internalRamIndex=0;
	externalRamIndex=0;
...
void * CECD100AppDlg::internalRamMalloc(unsigned long size)
{
	void * rc=NULL;

	size=(size+3)&(~0x3L);
	if((internalRamIndex+size)<=(8*1024))
	{
		rc=(void *)((unsigned long)internalRam + internalRamIndex);
		internalRamIndex+=size;
	}

	return rc;
}

unsigned long CECD100AppDlg::externalRamMalloc(unsigned long size)
{
	unsigned long rc=0;

	size=(size+3)&(~0x3L);
	if((externalRamIndex+size)<=(128*1024))
	{
		rc=EXTERNAL_RAM_BASE+ externalRamIndex;
		externalRamIndex+=size;
	}
...
{
	return pDlg->internalRamMalloc(size);
}

unsigned long stubMallocExternalRam(unsigned long size)
{
	return pDlg->externalRamMalloc(size);
}
...
unsigned char getUint8Array(unsigned long address, long index)
{
	...
	if((address>=EXTERNAL_RAM_BASE)&&(address<=(EXTERNAL_RAM_BASE+0x100000)))
	{
		//external RAM
		p=(unsigned char *)(((unsigned long)(pDlg->externalRam)) + (address-EXTERNAL_RAM_BASE) + index);
	}
	else if((address>=JAM_FILE_BASE)&&(address<=(JAM_FILE_BASE+0x100000)))
	{
		//Jam FILE
		p=(unsigned char *)(((unsigned long)(pDlg->jamBuffer)) + (address-JAM_FILE_BASE) + index);
	}
	else
	{
		//internal SRAM
		p=(unsigned char *)(address+index);
	}
...
long 	getInt32Array(unsigned long address, long index)
{
	...
	{
		//external RAM
		p=(long *)(((unsigned long)(pDlg->externalRam)) + (address-EXTERNAL_RAM_BASE) + index*4);
	}
	else if((address>=JAM_FILE_BASE)&&(address<=(JAM_FILE_BASE+0x100000)))
	{
		//Jam FILE
		p=(long *)(((unsigned long)(pDlg->jamBuffer)) + (address-JAM_FILE_BASE) + index*4);
	}
	else
	{
		//internal SRAM
		p=(long *)(address+index*4);
	}
