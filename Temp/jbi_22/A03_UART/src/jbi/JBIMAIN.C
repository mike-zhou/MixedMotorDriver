...
extern unsigned char ecd300GetJbcChar(unsigned long index);
...
void jbi_make_dword(unsigned char *buf, unsigned long num)
{
	...
	ecd300SetCharArray(buf, 0, (unsigned char) num);
	//buf[1] = (unsigned char) (num >> 8L);
	ecd300SetCharArray(buf, 1, (unsigned char) (num >> 8L));
	//buf[2] = (unsigned char) (num >> 16L);
	ecd300SetCharArray(buf, 2, (unsigned char) (num >> 16L));
	//buf[3] = (unsigned char) (num >> 24L);
	ecd300SetCharArray(buf, 3, (unsigned char) (num >> 24L));
}

unsigned long jbi_get_dword(unsigned char *buf)
{
	return
		(((unsigned long)ecd300GetCharArray(buf, 0)) |
		(((unsigned long)ecd300GetCharArray(buf, 1)) << 8L) |
		(((unsigned long)ecd300GetCharArray(buf, 2)) << 16L) |
		(((unsigned long)ecd300GetCharArray(buf, 3)) << 24L));
}
...
/****************************************************************************/
{
			...
			for (i = 0; i < (unsigned int) symbol_count; ++i)
			{
				...
				*/
				...
				if ((attributes[i] & 0x0c) == 0x04)
				{
					...
					ecd300SetLongArray(variables, i, value);
				}
				else if ((attributes[i] & 0x1e) == 0x0e)
				{
					...
					ecd300SetLongArray(variables, i, (long) jbi_malloc(uncompressed_size));

					if(ecd300GetLongArray(variables, i)==0L)
					{
					...
					else
					{
						...
						if (jbi_uncompress(
							program + data_section + value,
							variable_size[i],
							(unsigned char *)ecd300GetLongArray(variables, i),
							uncompressed_size,
							version)
							!= uncompressed_size)
						{
				...
				else if ((attributes[i] & 0x1e) == 0x0c)
				{
					...
					ecd300SetLongArray(variables, i, value + data_section + (long) program);
				}
				else if ((attributes[i] & 0x1c) == 0x1c)
				{
					/* initialized integer array */
					ecd300SetLongArray(variables, i, value + data_section);
				}
				else if ((attributes[i] & 0x0c) == 0x08)
				{
					...
					if (variable_size[i] > 0)
					{
						...
						ecd300SetLongArray(variables, i, (long) jbi_malloc(size));

						if (ecd300GetLongArray(variables, i) == NULL)
						{
							status = JBIC_OUT_OF_MEMORY;
						}
						else
						{
							...
							for (j = 0; j < size; ++j)
							{
								((unsigned char *)(ecd300GetLongArray(variables, i)))[j] = 0;
							}
						}
					}
					else
					{
						ecd300SetLongArray(variables, i, 0);
					}
				}
				else
				{
	...
	*/
	...
	while (!done)
	{
		...
		case 0x41: /* PSHV */
			stack[stack_ptr++] = ecd300GetLongArray(variables, args[0]);
			break;
		...
		case 0x44: /* NEXT */
			...
			*/
			IF_CHECK_STACK(3)
			{
				...
				long iterator = ecd300GetLongArray(variables, args[0]);
				int break_out = 0;
				...
				{
					ecd300SetLongArray(variables, args[0], iterator + step);
					pc = top + code_section;
					CHECK_PC;
				}
			}
		...
		case 0x4D: /* POPV */
			...
			*/
			IF_CHECK_STACK(1)
			{
				ecd300SetLongArray(variables, args[0], stack[--stack_ptr]);
			}
			break;
				...
				*/
					...
					((attributes[variable_id] & 0x9c) == 0x1c))
				{
					...
					*/
					...
					long_temp = ecd300GetLongArray(variables, variable_id);
					longptr_temp = (long *) jbi_malloc(count * sizeof(long));
					ecd300SetLongArray(variables, variable_id, (long) longptr_temp);

					if (ecd300GetLongArray(variables, variable_id) == NULL)
					{
						status = JBIC_OUT_OF_MEMORY;
						break;
					}
				...
				{
					longptr_temp = (long *) ecd300GetLongArray(variables, variable_id);
				...
				*/
					...
					((attributes[variable_id] & 0x9c) == 0x0c))
				{
					...
					*/
					...
					charptr_temp2 = (unsigned char *)ecd300GetLongArray(variables, variable_id);
					charptr_temp = jbi_malloc((unsigned int) long_temp);
					ecd300GetLongArray(variables, variable_id)= (long) charptr_temp;
				...
				{
					charptr_temp = (unsigned char *) ecd300GetLongArray(variables, variable_id);
		...
		case 0x52: /* IS   */
			...
			*/
			IF_CHECK_STACK(2)
			{
				...
				charptr_temp = (unsigned char *) ecd300GetLongArray(variables, args[0]);
			...
			*/
			IF_CHECK_STACK(1)
			{
				...
				else if ((attributes[variable_id] & 0x1f) == 0x1c)
				{
					...
					long_temp = ecd300GetLongArray(variables,  variable_id) + (4L * index);
				...
				if (long_temp > variable_size[variable_id])
				{
					...
					*/
					if ((attributes[variable_id] & 0x80) &&
						(ecd300GetLongArray(variables,  variable_id) != NULL))
					{
						jbi_free((void *) ecd300GetLongArray(variables,  variable_id));
						ecd300SetLongArray(variables,  variable_id, NULL);
					}

					/*
					*	Allocate a new buffer of the requested size
					*/
					ecd300SetLongArray(variables, variable_id, (long)jbi_malloc((unsigned int) long_temp));
					...
					else
					{
						...
						*/
						...
						charptr_temp = (unsigned char *)
							(ecd300GetLongArray(variables,  variable_id));
				...
				*/
				...
				if ((version > 0) && ((attributes[variable_id] & 0x9c) == 0x0c))
				{
					...
					*/
					...
					charptr_temp2 = (unsigned char *) ecd300GetLongArray(variables,  variable_id);
					charptr_temp = jbi_malloc((unsigned int) long_temp);
					ecd300SetLongArray(variables, variable_id, (long) charptr_temp);
				...
				charptr_temp = (unsigned char *) ecd300GetLongArray(variables, args[1]);
				charptr_temp2 = (unsigned char *)ecd300GetLongArray(variables, args[0]);
			...
			*/
			IF_CHECK_STACK(3)
			{
				...
				*/
				...
				charptr_temp = (unsigned char *) ecd300GetLongArray(variables,  args[0]);
				charptr_temp2 = (unsigned char *) ecd300GetLongArray(variables, args[1]);
		...
		case 0xC0: /* CMPA */
			...
			*/
			...
			{
				long a, b;
				unsigned char *source1 = (unsigned char *) ecd300GetLongArray(variables,args[0]);
				unsigned char *source2 = (unsigned char *) ecd300GetLongArray(variables, args[1]);
				unsigned char *mask    = (unsigned char *) ecd300GetLongArray(variables, args[2]);
	...
	*/
	if ((attributes != NULL) && (variables != NULL))
	{
		for (i = 0; i < (unsigned int) symbol_count; ++i)
		{
			if ((attributes[i] & 0x80) && (ecd300GetLongArray(variables,i) != NULL))
			{
				jbi_free((void *) ecd300GetLongArray(variables,i));
			}
		}
	}
