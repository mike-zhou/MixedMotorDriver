...
/****************************************************************************/
{
	...
	while (bits > 0)
	{
		databyte=ecd300GetCharArray(buffer, jbi_in_index);
		...
		while (i < data_length)
		{
			...
			if (jbi_read_packed(in, 1) == 0)
			{
				for (j = 0; j < DATA_BLOB_LENGTH; ++j)
				{
					if (i < data_length)
					{
						...
						ecd300SetCharArray(out, i, jbi_read_packed(in, CHAR_BITS));
						i++;
					}
				}
			}
			else
			{
				...
				for (j = 0; j < length; ++j)
				{
					...
					{
						//out[i] = out[i - offset];
						ecd300SetCharArray(out, i, ecd300GetCharArray(out, i-offset));
						i++;
					}
				}
			}
