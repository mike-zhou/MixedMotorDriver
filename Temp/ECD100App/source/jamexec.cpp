...
#else
		buffer = (char *)jam_malloc(uncompressed_length + 4);
...
/* syntax: DRSCAN <length> [, <data>] [COMPARE <array>, <mask>, <result>] ; */
	...
	*/
	if (status == JAMC_SUCCESS)
	{
		temp_array = (long *)jam_get_temp_workspace((count_value >> 3) + 4);
...
/****************************************************************************/
{
	...
	text_buffer = (char *)jam_malloc(JAMC_MAX_STATEMENT_LENGTH + 1024);
...
/****************************************************************************/
{
	...
	*/
	if (status == JAMC_SUCCESS)
	{
		temp_array = (long *)jam_get_temp_workspace((signal_count >> 3) + 4);
