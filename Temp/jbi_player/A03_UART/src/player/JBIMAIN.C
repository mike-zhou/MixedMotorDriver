...
void jbi_strncpy_ex(char *left, unsigned long rightInt8Ptr, long count)
{
	char ch;
			...
			for (i = 0; i < (unsigned long) symbol_count; ++i)
			{
				...
				*/
				...
				else if ((attributes[i] & 0x1e) == 0x0e)
				{
					...
					variables[i] = (long) jbi_malloc_ex(uncompressed_size);
				...
				else if ((attributes[i] & 0x0c) == 0x08)
				{
					...
					if (variable_size[i] > 0)
					{
						...
						variables[i] = (long) jbi_malloc_ex(size);
	...
	while (!done)
	{
			...
			IF_CHECK_STACK(2)
			{
				...
				*/
					...
					((attributes[variable_id] & 0x9c) == 0x1c))
				{
					...
					*/
					count = (unsigned long) variable_size[variable_id];
				...
				{
					longptr_temp = variables[variable_id];
				...
				*/
					...
					((attributes[variable_id] & 0x9c) == 0x0c))
				{
					...
					*/
					...
					charptr_temp2 =  variables[variable_id];
				...
				{
					charptr_temp = variables[variable_id];
				...
				if (long_temp > variable_size[variable_id])
				{
					...
					*/
					variables[variable_id] = (long)
						jbi_malloc_ex((unsigned long) long_temp);
			...
			*/
			IF_CHECK_STACK(3)
			{
				...
				*/
				...
				charptr_temp = variables[args[1]];
				charptr_temp2 = variables[args[0]];
			...
			*/
			IF_CHECK_STACK(3)
			{
				...
				*/
				...
				charptr_temp = variables[args[0]];
				charptr_temp2 = variables[args[1]];
		...
		case 0xC0: /* CMPA */
			...
			*/
			...
			{
				long a, b;
				unsigned long source1_uint8Ptr = variables[args[0]];
				unsigned long source2_uint8Ptr = variables[args[1]];
				unsigned long mask_uint8Ptr    = variables[args[2]];
