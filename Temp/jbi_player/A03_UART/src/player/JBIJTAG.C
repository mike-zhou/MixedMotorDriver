...
*/
unsigned long jbi_dr_preamble  = 0;
unsigned long jbi_dr_postamble = 0;
unsigned long jbi_ir_preamble  = 0;
unsigned long jbi_ir_postamble = 0;
unsigned long jbi_dr_length    = 0;
unsigned long jbi_ir_length    = 0;
...
)
{
	...
	if (status == JBIC_SUCCESS)
	{
		for (i = 0; i < count; ++i)
		{
			...
			if (preamble_data_uint8Ptr == NULL)
			{
				jbi_dr_preamble_data[i >> 3] |= (1 << (i & 7));
			}
			else
			{
				if (getUint8Array(preamble_data_uint8Ptr, j >> 3) & (1 << (j & 7)))
				{
					jbi_dr_preamble_data[i >> 3] |= (1 << (i & 7));
				}
				else
...
/****************************************************************************/
{
	...
	if (status == JBIC_SUCCESS)
	{
		for (i = 0; i < count; ++i)
		{
			...
			if (postamble_data_uint8Ptr == NULL)
			{
				jbi_dr_postamble_data[i >> 3] |= (1 << (i & 7));
			}
			else
			{
				if (getUint8Array(postamble_data_uint8Ptr, j >> 3) & (1 << (j & 7)))
				{
					jbi_dr_postamble_data[i >> 3] |= (1 << (i & 7));
				}
				else
...
/****************************************************************************/
{
	...
	for (i = 0L; i < preamble_count; ++i)
	{
		...
		{
			buffer[i >> 3L] &= ~(unsigned long) (1L << (i & 7L));
		}
	}
...
)
{
	...
	for (; i < k; ++i, ++j)
	{
		if (getUint8Array(target_data_uint8Ptr, j >> 3L) & (1L << (j & 7L)))
		{
			buffer[i >> 3L] |= (1L << (i & 7L));
		}
		else
...
/****************************************************************************/
{
	...
	for (i = start_index; i < k; ++i, ++j)
	{
		...
		{
			target_data[i >> 3] &= ~(unsigned long) (1 << (i & 7));
		}
	}
}
...
)
{
	...
	for (i = start_index; i < k; ++i, ++j)
	{
		if (buffer[j >> 3] & (1 << (j & 7)))
		{
			setUint8Array(target_data_uint8Ptr, i >> 3, getUint8Array(target_data_uint8Ptr, i >> 3)|(1 << (i & 7)));
		}
		else
		{
			setUint8Array(target_data_uint8Ptr, i >> 3, getUint8Array(target_data_uint8Ptr, i >> 3)&(~(unsigned long) (1 << (i & 7))));
		}
	}
}
...
{
	long start_code = 0;
	unsigned long alloc_chars = 0;
	unsigned long shift_count = jbi_ir_preamble + count + jbi_ir_postamble;
	JBI_RETURN_TYPE status = JBIC_SUCCESS;
...
)
{
	...
	if (status == JBIC_SUCCESS)
	{
		...
		*	Now extract the returned data from the buffer
		*/
		jbi_jtag_extract_target_data_ex
		(
			jbi_ir_buffer,
			out_data_uint8Ptr,
			out_index,
...
void jbi_free_jtag_padding_buffers(long reset_jtag)
