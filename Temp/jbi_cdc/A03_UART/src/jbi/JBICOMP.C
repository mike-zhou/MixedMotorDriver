...
static unsigned long jbi_in_length = 0L;
static unsigned long jbi_in_index = 0L;	/* byte index into compressed array */
static unsigned int jbi_bits_avail = CHAR_BITS;
...
/****************************************************************************/
{
	...
	while (bits > 0)
	{
		databyte=jbi_read_char_array(buffer_address, jbi_in_index);
...
/****************************************************************************/
{
	...
	{
		jbi_write_char_array(out_address, i, 0);
	}
	...
	for (i = 0; i < sizeof (in_length); ++i) 
	{
		data_length = data_length | ((unsigned long)
			jbi_read_packed(in_address, CHAR_BITS) << (i * CHAR_BITS));
	}
	...
	else
	{
		...
		while (i < data_length)
		{
			...
			if (jbi_read_packed(in_address, 1) == 0)
			{
				for (j = 0; j < DATA_BLOB_LENGTH; ++j)
				{
					if (i < data_length)
					{
						jbi_write_char_array(out_address, i, jbi_read_packed(in_address, CHAR_BITS));
						i++;
					}
				}
			}
			else
			{
				...
				offset = jbi_read_packed(in_address, jbi_bits_required((short) (i > match_data_length ? match_data_length : i)));
				length = jbi_read_packed(in_address, CHAR_BITS);

				for (j = 0; j < length; ++j)
				{
					if (i < data_length)
					{
						jbi_write_char_array(out_address, i, jbi_read_char_array(out_address, i-offset));
						i++;
					}
				}
			}
